...
export namespace financeUtils {
  ...
  const financeServer = {
    ...
    export async function getTokenGenerationFeeBT(tokenGenerationRequest: TokenGenerationRequest, financeServerUrl?: string, network: Network = 'mainnet'): Promise<BaseTransactionData>{
        const headers = {
          'Content-Type': 'application/json'
        };

        try {
            const { data } = await axios.post(`${financeServerUrl || financeServer[network].api}/admin/token/generate`, tokenGenerationRequest, { headers });
    ...
    export async function getTokenMintQuoteFee(tokenMintQuoteFeeRequest: TokenMintQuoteFeeRequest, financeServerUrl?: string, network: Network = 'mainnet'): Promise<TokenMintingFeeQuoteResponse> {
        try {
    ...
    export async function getTokenMintFee(tokenMintFeeRequest: TokenMintFeeRequest, financeServerUrl?: string, network: Network = 'mainnet'): Promise<BaseTransactionData>{
        try {
